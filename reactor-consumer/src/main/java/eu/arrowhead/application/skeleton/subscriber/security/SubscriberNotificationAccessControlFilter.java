...
public class SubscriberNotificationAccessControlFilter extends ArrowheadFilter {
	...
	public void setServerCN(final String serverCN) {
			...
			try {
				...
				if (eventTypeMap != null) {
					for (final String notificationUri : eventTypeMap.values()) {
						if (requestTarget != null && requestTarget.endsWith(
								SubscriberConstants.DEFAULT_EVENT_NOTIFICATION_BASE_URI + "/" + notificationUri)) {
							checkIfClientIsAnAllowedCoreSystem(getCertificateCNFromRequest(httpRequest),
									getServerCloudCN(serverCN), allowedCoreSystemsForSendingNotification,
									requestTarget);
						}
					}
				}
	...
	private String getCertificateCNFromRequest(final HttpServletRequest request) {
		final X509Certificate[] certificates = (X509Certificate[]) request
				.getAttribute(CommonConstants.ATTR_JAVAX_SERVLET_REQUEST_X509_CERTIFICATE);
	...
	// -------------------------------------------------------------------------------------------------
	protected void checkIfClientIsAnAllowedCoreSystem(final String clientCN, final String cloudCN,
			final CoreSystem[] allowedCoreSystems, final String requestTarget) {
		final boolean checkResult = checkIfClientIsAnAllowedCoreSystemNoException(clientCN, cloudCN, allowedCoreSystems,
				requestTarget);
	...
	// -------------------------------------------------------------------------------------------------
	protected boolean checkIfClientIsAnAllowedCoreSystemNoException(final String clientCN, final String cloudCN,
			final CoreSystem[] allowedCoreSystems, final String requestTarget) {
	...
	protected String getServerCloudCN(final String serverCN) {
		...
		final String[] serverFields = serverCN.split("\\.", 2); // serverFields contains: coreSystemName,
																// cloudName.operator.arrowhead.eu
